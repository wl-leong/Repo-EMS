...
-- Description:	Send email notification if LR have bookingNo
...
-- =============================================
--select * from lrHeader update lrHeader set apiStatus = '_READYLOAD_'  where lrHeaderId >= 216
-- EXEC [dbo].[SSP_Notification_LoadingRequestReadyToLoad] 
CREATE PROCEDURE [dbo].[SSP_Notification_LoadingRequestReadyToLoad]
AS
BEGIN
SET NOCOUNT ON
	BEGIN TRY
...
--2144	ReOpen
		...
		BEGIN	
		    ...
		    BEGIN
                ...
                SELECT h.lrHeaderId, h.lrName, li.earlyShipDate, containerSeq, lrContainerId, li.forwarderReplyDate, li.forwarderBookingNo
                INTO #container
                FROM lrContainer li
                ...
                WHERE li.lrHeaderId = @lrHeaderId
					...
					AND li.forwarderBookingNo <> ''

				DROP TABLE IF EXISTS #list;

				SELECT h.lrHeaderId, h.lrName, h.containerSeq, h.earlyShipDate, h.forwarderReplyDate, h.forwarderBookingNo, li.soHeaderId, li.invId, li.qty as lrQty, 
				...
				DECLARE @forwarderBookingNo VARCHAR(20) = (SELECT TOP 1 forwarderBookingNo FROM #list);
				DECLARE @forwarderReplyDate DATE = (SELECT TOP 1 forwarderReplyDate FROM #list);

				SET @tableHTML = 
						  '<H1>Ready to Load LR# : ' + @lrName + ' @ early Ship Date (' + ISNULL(CONVERT(VARCHAR, @lrShipDate, 106), '') + ')</H1>' +
						  '<H1>Forwarder Booking No : ' + @forwarderBookingNo + ' @ reply on (' + ISNULL(CONVERT(VARCHAR, @forwarderReplyDate, 106), '') + ')</H1>' +
            ...
            DECLARE @Subject VARCHAR(500) = '[READY TO LOAD] Loading Request # for ' + CONVERT(VARCHAR, getdate(), 106)

			SET @body = @body + '</HTML>'

			DECLARE @mailId BIGINT, @emailList VARCHAR(MAX);

			DECLARE @emailCategoryId INT = (SELECT emailCategoryId FROM md_EmailCategory WHERE actionName IN ('Ready LR'))
