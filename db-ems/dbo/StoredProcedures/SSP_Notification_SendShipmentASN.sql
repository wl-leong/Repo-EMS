...
-- Used By:	    EMS -> Shipping Module -> Shipping Monitoring -> Send ASN
--
-- Description : 
--
...
-- ==========================================================================================
-- 
CREATE PROCEDURE [dbo].[SSP_Notification_SendShipmentASN]
@BOL VARCHAR(80),
@updateBy INT
AS
BEGIN
	...
	BEGIN TRY	
		...
		SELECT shp.shipmentId, shp.shipId, shipmentDate, BOL, soName, soReferenceid, reference1, numberOfPallets, bolTotalPallets, shipmentWeight, boltotalshipmentweight, pickupAddrId, containerType, confirmedPickupDate, companyId, apiStatus, 0 as statusFlag, CAST('' as VARCHAR(150)) as returnMessage
		INTO #shipment
		...
		--BEGIN
		--	SET @returnMessage = 'BOL ' + @BOL + ' already process shipment ASN';
		--	THROW 60000, @returnMessage, 1
		--END

		IF (SELECT COUNT(1) FROM #shipment WHERE apiStatus NOT IN ('_RTG_','_ASN_')) > 0
		BEGIN
			SET @returnMessage = 'BOL ' + @BOL + ' is/are not able to Send ASN';
			THROW 60000, @returnMessage, 1
		END

		IF (SELECT COUNT(1) FROM #shipment WHERE apiStatus IN ('_RTG_','_ASN_') AND ISDATE(confirmedPickupDate) = 0) > 0
		BEGIN
			SET @returnMessage = 'BOL ' + @BOL + ' has/have empty Confirm Pickup Date';
			THROW 60000, @returnMessage, 1
		END
...
/** end: content prepration **/
		...
		SET @Subject = 'EMS BOL ' + @BOL + ' Advanced Shipment Notice ' + convert(VARCHAR, getdate(), 106)
		...
		SELECT '_SUCCESS_' as status, 'BOL '+ @BOL + ' has/have send shipment ASN email' AS returnMessage 
