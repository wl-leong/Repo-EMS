...
-- Used By:	    EMS -> Shipping Module -> Shipping Monitoring -> Send RTG
--
...
-- ==========================================================================================
-- EXEC [SSP_Notification_SendShipmentRTG] 'MSHP-FNP-2403005',1
CREATE PROCEDURE [dbo].[SSP_Notification_SendShipmentRTG]
@BOL VARCHAR(80),
@updateBy INT
AS
BEGIN
	...
	BEGIN TRY	
		...
		--DECLARE @BOL VARCHAR(80) = 'MSHP-FNP-2403005'
		DECLARE @returnMessage VARCHAR(1000);

		DROP TABLE IF EXISTS #shipment;

		SELECT shp.shipmentId, shp.shipId, shipmentDate, BOL, soName, scac, tracking, carrierPickUpDate, 
			soReferenceid, reference1, numberOfPallets, bolTotalPallets, shipmentWeight, boltotalshipmentweight, pickupAddrId, containerType, companyId, apiStatus
		INTO #shipment
		...
		--IF (SELECT COUNT(1) FROM #shipment WHERE apiStatus = '_RTG_') > 0
		--BEGIN
		--	SET @returnMessage = 'BOL ' + @BOL + ' already process shipment RTG';
		--	THROW 60000, @returnMessage, 1
		--END

		IF (SELECT COUNT(1) FROM #shipment WHERE apiStatus NOT IN ('_NEW_','_RTG_')) > 0
		BEGIN
			SET @returnMessage = 'BOL ' + @BOL + ' is/are not able to Send RTG';
			THROW 60000, @returnMessage, 1
		END

		IF (SELECT COUNT(1) FROM #shipment WHERE ISNULL(scac,'') = '') > 0
		BEGIN
			SET @returnMessage = 'BOL ' + @BOL + ' has/have empty SCAC';
			THROW 60000, @returnMessage, 1
		END

		IF (SELECT COUNT(1) FROM #shipment WHERE ISNULL(tracking,'') = '') > 0
		BEGIN
			SET @returnMessage = 'BOL ' + @BOL + ' has/have empty Tracking';
			THROW 60000, @returnMessage, 1
		END

		IF (SELECT COUNT(1) FROM #shipment WHERE ISNULL(carrierPickUpDate,'') = '') > 0
		BEGIN
			SET @returnMessage = 'BOL ' + @BOL + ' has/have empty Carrier PickUp Date';
			THROW 60000, @returnMessage, 1
		END
...
/** end: content prepration **/
		...
		SET @Subject = 'EMS BOL ' + @BOL + ' Routing Request ' + convert(VARCHAR, getdate(), 106)
		...
		--DECLARE @emailCategoryId INT = (SELECT emailCategoryId FROM md_EmailCategory WHERE companyId = @companyId AND menuLevel0Id = 9 AND actionName IN (''))
		...
		SELECT '_SUCCESS_' as status, 'BOL '+ @BOL + ' has/have send shipment RTG email' AS returnMessage 
