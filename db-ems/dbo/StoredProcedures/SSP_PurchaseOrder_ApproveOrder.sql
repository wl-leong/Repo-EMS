...
-- Used By:	    EMS -> PO Module -> PO Listing -> Approve PO
--
...
-- ==========================================================================================
-- EXEC [dbo].[SSP_PurchaseOrder_ApproveOrder] N'{"poList":[{"poId":"6"}, {"poId":"9"},{"poId":"3"}, {"poId":"5"}, {"poId":"8"}, {"poId":"4"}, {"poId":"7"}]}', 1
CREATE PROCEDURE [dbo].[SSP_PurchaseOrder_ApproveOrder]
...
BEGIN
	...
	BEGIN TRY	
		...
		BEGIN TRANSACTION
			...
			UPDATE poHeader SET
				poStatus = 1085,	
				poApprovalBy = @updateBy,
				poApprovalDate = getdate()
			FROM #order odr
		...
		SET @returnMessage = (SELECT 'PO# ' + STRING_AGG(CONVERT(VARCHAR(MAX),poName), ', ') + ' success approved.'
			                    FROM (SELECT DISTINCT poName 
                                        FROM #orderList 
			                            WHERE poStatus = 1080
                                    )g
